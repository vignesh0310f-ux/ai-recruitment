...
async function callClaude(systemPrompt, userMessage, maxTokens = 6000) {
...
{"roleTitle":"string","seniorityLevel":"string","domain":"string","hardSkills":["string"],"softSkills":["string"],"keyResponsibilities":["string"],"experienceLevel":"string"}`;
  const raw = await callClaude(system, `Parse this Job Description:\n\n${jdText}`, 1000);
...
Every question must be SPECIFIC to the role — not generic.`;
  const raw = await callClaude(system, `Generate assessment for this role:\n${JSON.stringify(parsedJD)}`, 6000);
  return safeParseJSON(raw);
}
...
Rules: all score values are integers 0-100. Be consistent.`;
  // Send only essential data to keep payload small
  const payload = {
  ...
  const raw = await callClaude(system, `Score candidate "${candidateName}":\n${JSON.stringify(payload)}`, 1500);
